...
#define TRANSACTIONS 40
#define TRANSACTIONS_PER_USER 10
#define USERS 4
#define SEED 8
...
void *use_atm(void *id_ptr) {
    ...
    for (int i = 0; i < TRANSACTIONS_PER_USER; i++) {
        ...
        while (true) {
            ...
            if (source == 1) {
                check_balance = balance_A - value; 
            } else if (source == 2) {
                check_balance = balance_B - value;
            } else {
                check_balance = 1;
            }

            if (value != 0 && check_balance >= 0) { // check if operation was done before
            ...
            } else if (value != 0 && check_balance < 0) { // print if account too low
                char *too_low;
                char *action_char;
                int tmp_balance;
                if (source == 1) {
                    too_low = "account A";
                    tmp_balance = balance_A;
                } else {
                    too_low = "account B";
                    tmp_balance = balance_B;
                }
                if (target != 0) {
                    action_char = "transfer";
                } else {
                    action_char = "withdraw";
                }
                printf("\ntransaction error for user %d : not enough credit to %s from %s, courrent balance: %d, need: %d \n", *id, action_char, too_low, tmp_balance, value);
                if (tmp_balance - value > 0)
                {
                    printf("\n XXX \n");printf("\n+-----------------------------------------------------------------------------------+\n");printf("\n+-----------------------------------------------------------------------------------+\n");printf("\n+-----------------------------------------------------------------------------------+\n");printf("\n+-----------------------------------------------------------------------------------+\n");
                }
            }
